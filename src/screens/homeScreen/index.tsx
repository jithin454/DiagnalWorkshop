...
function HomeScreen(): React.JSX.Element {
  ...
  const regex = useMemo(
    () => new RegExp(`^.{0,30}${searchText}.{0,}`, 'i'),
    [searchText],
  );
  const logo = ENV.log;
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `${ENV.apiUrl}CONTENTLISTINGPAGE-PAGE${state.page}.json`,
      );
      ...
      if (
        ...
        state.listData.length + parseInt(pageSizeReturned)
      ) {
  ...
  const filteredData = useMemo(
    () => state.listData.filter(item => regex.test(item.name)),
    [state.listData, regex],
  );
